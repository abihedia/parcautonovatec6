...
class moyennecompteur(models.Model):
    ...
    moy_compteur_Noir = fields.Integer('Moy compteur NB')
    moy_compteur_Couleur = fields.Integer('Moy compteur Couleur')
...
class FleetContINHERITFact(models.Model):
    ...
    ########## smart button to compteur
    count_moy_coleur_noir = fields.Integer(string="Compteur moyen", compute="compute_moy_count")

    def compute_moy_count(self):
            for rec in self:
                order_count = self.env['moycompteurnoircol'].search_count([('fleet_id', '=', rec.id)])
                rec.count_moy_coleur_noir = order_count
